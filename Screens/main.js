...
const Main = () => {
  ...
  const [wfn, setwfn] = useState("http://192.168.1.9:80/wav_files/2024-02-18-7777.wav");
  const [mfn, setmfn] = useState("http://192.168.1.9:80/midi_files/2024-02-18-7777.midi");
  ...
  const downloadMidiFile = () => {
    ...
    let options = {
      ...
      addAndroidDownloads: {
        path:
        RNFetchBlob.fs.dirs.DownloadDir + 's',
        description: 'downloading file...',
        notification: true,
        // useDownloadManager works with Android only
        useDownloadManager: true,   
  ...
  const generateChords = async() =>{
    if(key && Scale && arp){
      ...
      Alert.alert(format('Fill the required fields :- {0} {1} {2}',(!key) ?"Key":"", (!Scale) ?"Scale":"", (!arp) ?"Arp":""));
    }
  }
